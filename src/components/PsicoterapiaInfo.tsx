...
const ICON_EXIT_TRANSITION = { duration: 0.5, ease: "easeOut" } as const;
      ...
      <div className="relative w-16 h-16 sm:w-20 sm:h-20 mb-1">
        <AnimatePresence mode="wait">
          <motion.svg
            key={replay[index]}
            viewBox={viewBox}
            className="absolute inset-0 w-full h-full text-[#c9b8dc] group-hover:text-[#a18db8] transition-colors"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            exit={{ opacity: 0, transition: ICON_EXIT_TRANSITION }}
          >
            {children}
          </motion.svg>
        </AnimatePresence>
...
export default function PsicoterapiaInfo() {
  ...
  const [replay, setReplay] = useState<number[]>(Array(9).fill(0));
  const bump = (i: number) => setReplay(r => r.map((v, j) => (j === i ? v + 1 : v)));
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-16 lg:gap-x-20 lg:gap-y-12 items-start">
          ...
          <div className="w-full relative order-3 lg:col-start-1 lg:row-start-2">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-10 gap-x-6 justify-items-center text-[#9e9282]">
              ...
              <IconCard index={0} replay={replay} bump={bump} viewBox="0 0 1208.55 800.47" label={<>Humor e Regulação<br />Emocional</>}>
                <motion.path
              ...
              <IconCard index={1} replay={replay} bump={bump} viewBox="0 0 1200.32 861.93" label={<>Gênero e<br />atravessamentos</>}>
                <motion.circle
                  cx="597.64" cy="505.28" r="323.95"
                  fill="none" stroke="currentColor" strokeWidth="28" strokeLinecap="round" strokeLinejoin="round"
              ...
              {/* Sexualidade */}
              <IconCard index={2} replay={replay} bump={bump} viewBox="0 0 713.88 846.03" label="Sexualidade">
                <motion.path
                  ...
                  fill="none" stroke="currentColor" strokeWidth="18" strokeLinecap="round" strokeLinejoin="round"
              ...
              <IconCard index={3} replay={replay} bump={bump} viewBox="0 0 751.8 883.62" label={<>Sofrimento<br />Existencial</>}>
                <motion.path
              ...
              {/* Não Monogamia */}
              <IconCard index={4} replay={replay} bump={bump} viewBox="0 0 957.65 831.69" label="Não Monogamia">
                <motion.path
              ...
              <IconCard index={5} replay={replay} bump={bump} viewBox="0 0 1468.95 456.64" label={<>Vínculos e<br />Comunicação</>}>
                <motion.path
              ...
              <IconCard index={6} replay={replay} bump={bump} viewBox="0 0 752.5 737.5" label={<>Terapia de Casal<br />e seus acordos</>}>
                <motion.path
              ...
              {/* Autoconhecimento */}
              <IconCard index={7} replay={replay} bump={bump} viewBox="0 -85 784.03 934.45" label="Autoconhecimento">
                <motion.circle
              ...
              {/* Ansiedade */}
              <IconCard index={8} replay={replay} bump={bump} viewBox="0 0 1331.11 861" label="Ansiedade">
                <motion.path
